...
def Load2():
    ...
    df_ESG_scores = spark.read \
        .format("jdbc") \
        .option("url", postgres_uri) \
        .option("dbtable", ESG_scores) \
        .option("user", user) \
        .option("password", password) \
        .option("driver", "org.postgresql.Driver") \
        .load()

    df_stockprices = spark.read \
        .format("jdbc") \
        .option("url", postgres_uri) \
        .option("dbtable", stockprices) \
        .option("user", user) \
        .option("password", password) \
        .option("driver", "org.postgresql.Driver") \
        .load()

    df_financials = spark.read \
        .format("jdbc") \
        .option("url", postgres_uri) \
        .option("dbtable", financials) \
        .option("user", user) \
        .option("password", password) \
        .option("driver", "org.postgresql.Driver") \
        .load()

    df_glassdoor_reviews = spark.read \
        .format("jdbc") \
        .option("url", postgres_uri) \
        .option("dbtable", glassdoor_reviews) \
        .option("user", user) \
        .option("password", password) \
        .option("driver", "org.postgresql.Driver") \
        .load()
    ...
    query1 = "SELECT glassdoor_reviews.ticker, AVG(glassdoor_reviews.ratings) \
                FROM glassdoor_reviews \
                GROUP BY glassdoor_reviews.ticker \
                ORDER BY AVG(glassdoor_reviews.ratings) \
                LIMIT 10"

    # Query 2: Top 10 companies by highest average stockprices from 2012 to 2021 
    query2 = "SELECT stockprices.ticker, company.security, ROUND((AVG(stockprices.high) + AVG(stockprices.low)/ 2),2) as Avg_stockprices\
                ...
                WHERE stockprices.date <= '2021-12-31' \
                GROUP BY stockprices.ticker,company.security \
                ORDER BY Avg_stockprices DESC \
                LIMIT 10"

    # Query 3: Top 10 companies by the highest average ESG scores in 2021 
    query3 = "SELECT ESG_scores.ticker, company.security, AVG(ESG_scores.total_score) as Avg_ESG_Score \
                FROM ESG_scores \
                INNER JOIN company ON ESG_scores.ticker = company.ticker \
                WHERE ESG_scores.date > '2021-01-01' AND ESG_scores.date <= '2021-12-31' \
                GROUP BY ESG_scores.ticker,company.security \
                ORDER BY Avg_ESG_Score DESC \
                LIMIT 10"
    
    # Query 4: Top 5 headquartered locations for SP&500 companies
    query4 = "SELECT COUNT(company.security) As number, company.headquarters \
                FROM company \
                GROUP BY company.headquarters \
                ORDER BY number DESC \
                LIMIT 5"
    
    # Query 5: Top 10 stocks by prices change (update daily)  
    query5 = "SELECT stockprices.ticker,company.security, stockprices.date,ROUND((MAX(stockprices.closing)- MIN(stockprices.opening)) / MIN(stockprices.opening) * 100, 2) as stockprice_change \
                FROM stockprices \
                INNER JOIN company ON stockprices.ticker = company.ticker \
                WHERE CAST(stockprices.date AS DATE) = CAST(current_date()-1 AS DATE) \
                GROUP BY stockprices.ticker, company.security, stockprices.date\
                ORDER BY stockprice_change DESC \
                LIMIT 10"
    ...
    s3csv.put_object(Key='Queries/Top 10 companies by highest average stockprices from 2012 to 2021.csv', Body=csv_buffer2.getvalue(),Bucket=BUCKET_NAME)
    csv_buffer3 = StringIO()
    stats3.to_csv(csv_buffer)
    ...
    stats5.to_csv(csv_buffer)
    s3csv.put_object(Key='Queries/Top 5 headquartered locations for SP&500 companies.csv', Body=csv_buffer5.getvalue(),Bucket=BUCKET_NAME)
    
    # Send query file to investors by Gmail
    def send_email(send_to, subject, df):
        send_from = "alysalws2@gmail.com"
        password = "Al080795900300" # This is only a temporary password. It will be changed on 8 May 2022.
        message = """\
        <p><strong>Here's the Top 10 stocks by prices change (updated daily)  </strong></p>
        <p><br></p>
    ...
    send_email(['alysalws@gmail.com'],'Top 10 Stocks by price change', stats5)
