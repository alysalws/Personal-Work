...
def Load2():
    ...
    query1 = "SELECT glassdoor_reviews.ticker, company.security, AVG(glassdoor_reviews.ratings) \
                FROM glassdoor_reviews \
                INNER JOIN company ON glassdoor_reviews.ticker = company.ticker \
                GROUP BY glassdoor_reviews.ticker,company.security \
    ...
    query5 = "SELECT stockprices.ticker,company.security, stockprices.date, ROUND((MAX(stockprices.closing)- MIN(stockprices.opening)) / MIN(stockprices.opening) * 100, 2) as stockprice_change, stockprices.opening, stockprices.closing \
                FROM stockprices \
                JOIN company ON stockprices.ticker = company.ticker \
                WHERE CAST(stockprices.date AS DATE) = CAST(current_date()-3 AS DATE) \
                GROUP BY stockprices.ticker, company.security, stockprices.date,  stockprices.opening, stockprices.closing \
    ...
    def send_email(send_to, subject, df):
        ...
        message = """\
        <p><strong> Hi, here's the updated list of Top 10 Stocks by stockprice changes. Thanks! </strong></p>
        <p><br></p>
